...
# NOTE change it to be female 
    with open(os.path.join(dataset_dir, "female_mentions_persons_with_gender.txt")) as f:
...
def _load_documents(
    ...
    with open(tsv_file, "r") as f:
        ...
        for line in f:
            ...
            is_person = determine_person(mention, person_names)
            if is_person:
                mentions_name.append(mention.title)
                doc_id2mentions[doc_id].append(mention)
                mention_index += 1
        # save name
        with open ('./examples/entity_disambiguation/entity_disambiguation/mentions_name.txt', 'w') as f:
            for name in mentions_name:
                f.write(name+'\n')
# NOTE: change herem rule out not person mentions
...
def determine_person(mention: Mention, person_names: AbstractSet[str]):
    if mention.title in person_names or  mention.text in person_names:
        return True 
    else:
        return False
